...
void bf_pmull_example(test_ctr *ctr) {
	...
	bf_polyx2 expected = concat_bf_poly(expected0, expected1);
	
	//Act
	bf_polyx2 actual = bf_pmull(a, b);
...
void bf_pmull_commutative(test_ctr *ctr) {
	...
	//Act
	bf_polyx2 ab = bf_pmull(a,b);
	bf_polyx2 ba = bf_pmull(b,a);
...
void bf_psquare_example(test_ctr *ctr) {
	...
	bf_polyx2 expected = concat_bf_poly(expected0, expected1); //z^252 + z^178 + z^6
...
void bf_psquare_one_is_one(test_ctr *ctr) {
	...
	poly64x2_t one = {1, 0};
	poly64x2_t zero = {0, 0};
	bf_polyx2 one_extended = concat_bf_poly(one, zero);
	
	//Act
	bf_polyx2 result = bf_psquare(one);
...
void bf_psquare_zero_is_zero(test_ctr *ctr) {
	...
	//Act
	bf_polyx2 result = bf_psquare(zero);
...
void bf_psquare_crosscheck_pmull(test_ctr *ctr) {
	...
	//Act
	bf_polyx2 result_psquare = bf_psquare(a);
	bf_polyx2 result_pmull = bf_pmull(a, a);
...
void bf_psquare_freshmans_dream(test_ctr *ctr) {
	...
	//Act
	bf_polyx2 apow2 = bf_psquare(a);
	bf_polyx2 bpow2 = bf_psquare(b);
	bf_polyx2 apow2_plus_bpow2 = concat_bf_poly(bf_add(apow2.p0, bpow2.p0), bf_add(apow2.p1, bpow2.p1));
	bf_polyx2 aplusb_pow2 = bf_psquare(bf_add(a, b));
...
void bf_red_example_twowords(test_ctr *ctr) {
	...
	poly64x2_t zero = {0,0};
	bf_polyx2 a = concat_bf_poly(a0, zero); //z^127 + z^126 + z^55 + 1
...
void bf_red_example_threewords(test_ctr *ctr) {
	...
	bf_polyx2 a = concat_bf_poly(a0, a1); //z^169 + z^129 + z^126 + z^55 + 1
...
void bf_red_example_fourwords(test_ctr *ctr) {
	...
	bf_polyx2 a = concat_bf_poly(a0, a1); //z^252 + z^198 + 1
...
void bf_red_reduction_polynomial_reduces_to_zero(test_ctr *ctr) {
	...
	poly64x2_t zero = {0,0};
	bf_polyx2 f_extended = concat_bf_poly(f, zero);
